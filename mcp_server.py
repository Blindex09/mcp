...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "COVERAGE: a11y_page_map gives FACTS about ALL content (headings, images, links, forms, tables, media, iframes, reading order, hover rules), and the dossier crosses Shadow DOM and iframes; a11y_coverage lists what was and was NOT covered (always include it in reports); a11y_crawl scans several pages of a site (facts only, robots.txt respected, GET only). APPROVAL (never a flat refusal): on non-local sites, requests that change data (POST/PUT/PATCH/DELETE, form submits) are HELD and shown (method, URL, field names, never values) until the person answers via a11y_approve; local dev sites run with approval off; allow_mutations=auto|ask|allow|block. LEARNING: autonomous runs teach the server (a11y_learnings / a11y_forget). "
